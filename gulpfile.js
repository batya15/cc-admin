...
require('./gulp/clean')('clean', gulp, ["build", "release", config.path.temp], config.path.src);
//Удаление папки билда и всех зависимостей npm и bower
require('./gulp/clean')('cleanHard', gulp, ["build", "release", config.path.bower_components, config.path.vendor_js, config.path.temp, config.path.node_modules], config.path.src);
//Скачивание всех зависимостей
function installModules(configJson) {
    return gulp.src([configJson])
        .pipe(install());
}
function installModulesNPM() {
    return installModules(config.npm)
}
function installModulesBower() {
    return installModules(config.bower)
}
...
function compileStyle() {
    return gulp.src(config.path.scssFiles)
        ...
        .pipe(sass({quiet: true, includePaths: ['src/static']}))
        .pipe(autoprefixer({
...
function registerWatchers() {
    watch(config.staticFiles, {verbose: true, name: 'copy-changed-files', base: 'src/static'}, function (files, done) {
...
//Минимизация JS
function jsMin() {
    return gulp.src([config.path.build + '/**/*.js', "!**/test/**", "!**/css.js", "!**/css-builder.js"], {base: 'build/static'})
        .pipe((through.obj(function (file, enc, cb) {
